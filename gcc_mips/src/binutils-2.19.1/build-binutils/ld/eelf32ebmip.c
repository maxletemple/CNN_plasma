...
static void gldelf32ebmip_before_parse (void);
static void gldelf32ebmip_after_open (void);
static void gldelf32ebmip_before_allocation (void);
static bfd_boolean gldelf32ebmip_place_orphan (asection *s);
static void gldelf32ebmip_finish (void);


static void
gldelf32ebmip_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
#define is_mips_elf(bfd)				  (bfd_get_flavour (bfd) == bfd_target_elf_flavour	   && elf_tdata (bfd) != NULL				   && elf_object_id (bfd) == MIPS_ELF_TDATA)
...
static void
gldelf32ebmip_before_parse (void)
{
...
static bfd_boolean
gldelf32ebmip_load_symbols (lang_input_statement_type *entry)
{
  int class = 0;
...
static void
gldelf32ebmip_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32ebmip_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32ebmip_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needed != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32ebmip_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
...
static bfd_boolean
gldelf32ebmip_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf32ebmip_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf32ebmip_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf32ebmip_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32ebmip_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf32ebmip_write_build_id_section (bfd *abfd)
{
...
gldelf32ebmip_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
    {
      ...
      size = gldelf32ebmip_id_note_section_size (abfd, &link_info);
      if (size == 0)
	{
      ...
      else
	{
	  ...
	  if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		...
		= &gldelf32ebmip_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32ebmip_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32ebmip_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  ...
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32ebmip_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32ebmip_find_exp_assignment (etree_type *exp)
{
...
static void
gldelf32ebmip_find_statement_assignment (lang_statement_union_type *s)
{
...
static void
gldelf32ebmip_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
...
static bfd_boolean
gldelf32ebmip_place_orphan (asection *s)
{
...
static void
gldelf32ebmip_finish (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);

  gldelf32ebmip_map_segments (need_layout);
  finish_default ();
}

static char *
gldelf32ebmip_get_script (int *isfile)
{
  *isfile = 0;
...
static bfd_boolean
gldelf32ebmip_handle_option (int optc)
{
  switch (optc)
    {
    default:
...
static void
gldelf32ebmip_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf32ebmip_emulation =
{
  ...
  NULL,
  gldelf32ebmip_add_options,
  gldelf32ebmip_handle_option,
  NULL,
  gldelf32ebmip_list_options,
  gldelf32ebmip_load_symbols,
  NULL,
  NULL
};
